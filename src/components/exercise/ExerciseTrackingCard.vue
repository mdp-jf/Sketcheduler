      ...
      <div v-else class="space-y-3">
        ...
        >
          <div class="flex justify-between mb-1">
            ...
            <span class="text-sm"
              >{{ category.completed }} / {{ category.total }}</span
            >
          </div>
...
import type { UserExerciseProgress } from "../../types/models";
...
const exerciseCompletionPercentage = computed(() => {
  ...
  return Math.round(
    (props.completedExercises.length / props.totalExercises) * 100,
  );
});
...
const averageRating = computed(() => {
  const exercisesWithRating = props.completedExercises.filter(
    (exercise) =>
      exercise.self_rating !== null && exercise.self_rating !== undefined,
  );
