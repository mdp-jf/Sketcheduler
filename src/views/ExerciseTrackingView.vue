<template>
    <div class="exercise-tracking-container">
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-lg shadow p-6">
            ...
            <div v-if="suggestedExercises.length === 0" class="text-gray-500 italic py-4">
              No suggested exercises available
            </div>
            
            <div v-else class="space-y-4">
              <div v-for="exercise in suggestedExercises" :key="exercise.id" class="border-b pb-4 last:border-b-0 last:pb-0">
                <div class="flex justify-between">
                  <div>
                    <h3 class="font-medium">{{ exercise.title }}</h3>
                    <p class="text-sm text-gray-600 mt-1">{{ truncateText(exercise.description, 60) }}</p>
                  </div>
                  <button
          ...
          <div v-if="warmupExercises.length > 0" class="bg-white rounded-lg shadow p-6 mt-6">
            <h2 class="text-xl font-bold mb-4">Daily Warmup</h2>
            <p class="text-gray-600 mb-4">Start your practice with a quick warmup exercise</p>
            
            <div class="border rounded-lg p-4 bg-yellow-50">
              <h3 class="font-medium">{{ randomWarmupExercise?.exercises?.title }}</h3>
              <p class="text-sm text-gray-600 mt-1">{{ randomWarmupExercise?.exercises?.description }}</p>
              <button
      ...
      <!-- All Exercises Modal -->
      <div v-if="showAllExercises" class="fixed inset-0 flex items-center justify-center z-50">
        <div class="fixed inset-0 bg-black opacity-50" @click="showAllExercises = false"></div>
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-4xl z-10 relative max-h-[90vh] overflow-y-auto">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr v-for="exercise in filteredExercises" :key="exercise.id" class="hover:bg-gray-50">
                  <td class="px-4 py-3">
                    <div class="font-medium text-gray-900">{{ exercise.title }}</div>
                    <div class="text-xs text-gray-500 mt-1">{{ truncateText(exercise.description, 60) }}</div>
                  </td>
                  ...
                  <td class="px-4 py-3">
                    ...
                    >
                      {{ isExerciseCompleted(exercise.id) ? 'Completed' : 'Not Completed' }}
                    </span>
                  </td>
                  <td class="px-4 py-3 text-sm text-gray-500">
  ...
  const suggestedExercises = computed(() => {
    ...
    const notCompleted = allExercises.filter(exercise => !completedIds.has(exercise.id));
  ...
  const filteredExercises = computed(() => {
    ...
    if (exerciseFilter.value) {
      ...
      result = result.filter(
        ex => ex.title.toLowerCase().includes(searchLower) || 
  ...
  const getCompletionDate = (exerciseId: number): string | null => {
    const exercise = completedExercises.value.find(ex => ex.exercise_id === exerciseId);
  ...
  const loadData = async () => {
    try {
      ...
      if (lessonsStore.lessons.length > 0) {
        ...
        const exercisesResult = await exercisesStore.fetchExercisesByLessonId(firstLessonId);
        
        if (!exercisesResult.success) {
          throw new Error(String(exercisesResult.error) || "Failed to load exercises");
        }
      }
      ...
      if (!trackingResult.success) {
        throw new Error(String(trackingResult.error) || "Failed to load exercise tracking data");
      }
